...
if $FLOATING; then
  ...
  ACTIVE_MONITOR_WIDTH=$(($(echo "$ACTIVE_MONITOR" | jq -r ".width") * 100))
  ACTIVE_MONITOR_HEIGHT=$(($(echo "$ACTIVE_MONITOR" | jq -r ".height") * 100))
  ACTIVE_MONITOR_SCALE=$(awk "BEGIN {printf $(echo "$ACTIVE_MONITOR" | jq -r ".scale") * 100}")
  ...
  "t" | "top")
    ...
    POSITION_xy="$(($ACTIVE_MONITOR_X + (($ACTIVE_MONITOR_WIDTH_SCALED * (100 - $WIDTH_xy)) / 200))) $(($GAP + $ACTIVE_MONITOR_Y))"
    ;;
  "b" | "bottom")
    if [[ -n $WIDTH ]]; then WIDTH_xy=$WIDTH; else WIDTH_xy="100"; fi
    if [[ -n $HEIGHT ]]; then HEIGHT_xy=$HEIGHT; else HEIGHT_xy="50"; fi
  ...
  *)
  ...
  hyprctl --batch "keyword windowrule float,class:^$CLASS$ ; keyword windowrule move $POSITION_xy,class:^$CLASS$ ; keyword windowrule size $WIDTH_xy% $HEIGHT_xy%,class:^$CLASS$"
fi
...
if [[ -n $(hyprctl clients -j | jq -r ".[] | select(.class==\"$CLASS\" and .workspace.id!=$ACTIVE_WORKSPACE)") ]]; then
  if [[ $FOCUS == false ]]; then
    ...
    hyprctl --batch "dispatch resizewindowpixel exact $WIDTH_xy% $HEIGHT_xy%,class:^$CLASS$ ; dispatch movewindowpixel exact $POSITION_xy,class:^$CLASS$"

...
Scope {
    ...
    function buildWorkspaces() {
        ...
        for (const ws of Hyprland.workspaces.values) {
            ...
            result[mon].push({
                ...
                active:   ws.active,
                focused:  ws.focused,
                appIds:   root.workspaceAppsById[ws.id] ?? [],
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                try {
                    ...
                    const activeByMonitor = {}
                    const appsByWorkspace = {}
                    for (const c of clients) {
                        const monName = root.monitorIdToName[c.monitor]
                        if (monName && !activeByMonitor[monName])
                            activeByMonitor[monName] = { title: c.title, appId: c.class }

                        const workspaceId = c.workspace?.id
                        if (!workspaceId || !c.class) continue
                        if (!appsByWorkspace[workspaceId]) appsByWorkspace[workspaceId] = []
                        appsByWorkspace[workspaceId].push({
                            appId: c.class,
                            area: (c.size?.[0] ?? 0) * (c.size?.[1] ?? 0)
                        })
                    }
                    for (const workspaceId in appsByWorkspace) {
                        appsByWorkspace[workspaceId].sort((a, b) => b.area - a.area)
                        appsByWorkspace[workspaceId] = appsByWorkspace[workspaceId].map(client => client.appId)
                    }
                    root.activeWindowByMonitor = activeByMonitor
                    root.workspaceAppsById = appsByWorkspace
                    root.buildWorkspaces()
                } catch(e) {}
            }
        }
    }
    ...
    Connections {
        ...
        function onRawEvent(event) {
            ...
            if (["openwindow", "closewindow", "movewindow", "activewindow", "windowtitle"].includes(event.name))
                root.refreshClients()
